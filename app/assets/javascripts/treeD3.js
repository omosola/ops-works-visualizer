/*$(document).ready(function() {
	...
	var treeData = window.data[0].layers; // will normally be pulled from the API using $.getJSON();
	debugger;
	var pie = d3.layout.pie();

	// CURRENTLY WORKING ON ONE DONUT TO REPRESENT ONE LAYER OF A STACK
	...
	// bind data array to the data
	// but first pass it through pie
	...
	// create a path from each arc object
	arcs.append("path")
		.attr("d", arc)
		.attr("fill", function(d) { return color(d.data) });

	arcs.append("text")
		.attr("transform", function(d) { return "translate("+ arc.centroid(d) + ")"; }) // positions the labels
